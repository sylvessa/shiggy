...
nat8 fat32_log = 1; // set to 0 to disable logs

static nat32 fat_start_lba;
static nat32 cluster2_lba; // first data cluster
static nat32 sectors_per_fat;
static nat32 total_clusters;
static nat32 next_free_cluster;

static nat8 fat[SECTOR_SIZE*8]; // simple FAT copy (first 8 sectors)
...
static void load_root_dir() {
    ...
    for (int i = 0; i < MAX_ROOT_ENTRIES; i++) {
        ...
        root_dir[i].first_cluster_low = buffer[offset + 26] | (buffer[offset + 27] << 8);
        root_dir[i].file_size = buffer[offset + 28] | (buffer[offset + 29] << 8);
    }
